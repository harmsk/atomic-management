...
export default {
  ...
  readConfigFile(configName) {
    ...
    if(this.configContents &&
       this.configContents.core &&
        this.configContents.core.disabledPackages &&
         contents.core &&
          contents.core.disabledPackages) {
      if(!this.compareArrays(contents.core.disabledPackages,this.configContents.core.disabledPackages)) {
        this.askReload()
      }
    }
    ...
    const notInstalledPackageNames = contents.core.disabledPackages.filter(packageName => !installedPackageNames.has(packageName));
        ...
        Object.keys(contents).forEach((namespace) => {
            Object.keys(contents[namespace]).forEach((key) => {
                if(!this.configuredFields.has(key)) {
                    ...
                    this.configuredFields = this.configuredFields.set(keyPath, contents[namespace][key])
                }
            })
        })
    }
    atom.config.resetProjectSettings(contents, atom.enablePersistence || configName || atom.config.mainSource)
  },
   ...
   *
   * @param {string[]} packageNames The names of packages that are not installed currently
   */
