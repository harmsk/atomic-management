...
export default {
  ...
  readConfigFile(configName) {
    ...
    const currDisabled = this.hasPackageDisable(this.configContents);
    const newDisabled = this.hasPackageDisable(contents);
  ...
  setConfig() {
    ...
    switch (this.projectConfigs.length) {
      ...
      default:
        ...
        if (
          this.defaultConfig &&
          projectPaths.some(defaultCheck(this.defaultConfig))
        ) {
