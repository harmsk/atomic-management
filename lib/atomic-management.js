...
// requires fs, glob, season, merge-json, path libraries and assigned them as constants.
const fs = require('fs');
...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(
      ...
      atom.config.onDidChange("atomic-management.isActive",
      ...
      atom.workspace.observeTextEditors(editor => {
        editor.onDidSave(e => {
          ...
          if (this.enabledProjects.length === 0 || (this.enabledProjects.includes(e.path) && e.path.endsWith(FILE_EXTENSION))) {
            this.setConfig();
          }
        });
      })
   ...
   * `atomic-management.isActive`. If the package is toggled off, the global
   ...
   *
   * @param {Boolean} newValue The new value of `atomic-management.isActive`
   */
  enabledChanged(newValue){
    if(!newValue){
      ...
      atom.config.set('atomic-management.defaultConfig', null)
      this.enabledProjects = [];
  ...
  toggle() {
    let old = atom.config.get('atomic-management.isActive');
    atom.config.set('atomic-management.isActive', !old);
  },
   ...
   */
  setConfig() {
    if (!atom.config.get('atomic-management.isActive')) return;
...
onCheck(index, checked){
  ...
  readConfigs(newProjectConfigsArray=this.projectConfigs, makeDefault=false) {
    ...
    newProjectConfigsArray.forEach(configName => {
      ...
      for (let toplevel in contents) {
        for (let pname in contents[toplevel]) {
          if (pname !== "core" && pname !== "editor" && !installedPackageNames.has(pname)) {
    ...
    //for test
    savedConfigFile = path.join(atom.project.getPaths()[0], "/spec/test.cson")
    CSON.writeFile(savedConfigFile, atom.config.get(), () => console.log("error"))
  },
  ...
  checkPackages(packagesMap) {
    ...
    this.runApmCommand("view", spacelessPackageNames)
      .then(([successful, unsuccessful]) => {
        ...
        if (badPackageNames.length > 0) {
          ...
          badPackageNames.forEach(name => {
            packagesMap.get(name).forEach(configFile => badPackageConfigFiles.add(configFile));
          });

          this.alertBadPackages(badPackageNames, Array.from(badPackageConfigFiles));
        }
      });
  },
  ...
  */
  installPackages(packageNames) {
    ...
    this.runApmCommand("install", packageNames)
      .then(([successful, unsuccessful]) => {
  ...
  runApmCommand(apmCommand, packageNames) {
    ...
    return Promise.all(apmPromises)
      .then(allResults => {
