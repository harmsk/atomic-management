...
<<<<<<< HEAD
  /**
   * Toggles Atomic Management on/off.
   */
=======
  /* Toggles to enable/disable the package manually. If the package is disabled, then it will look for the correct configuration file, apply the configuration specified in the configuration file, and notify users; if the package is enabled, it disables the package and notifies users.
  */
>>>>>>> master
  toggle() {
    if (this.isEnabled) {
...
<<<<<<< HEAD
  /**
   * Reads the atomconfig file (in either cson or json format), processes the
   * file (alerts the user for newly disabled packages, newly configured
   * packages, etc.), and sets the local project config.
   * 
   * @param {String} configName The path to a configuration file that ends in
   *                            ".cson" or ".json"
=======
  toggleEnforced() {
...
>>>>>>> master
  ...
  /**
   * Opens the atomconfig file that is being applied to the current window.
   */
  openConfigFile() {
