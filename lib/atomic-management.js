...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(
      ...
      atom.config.onDidChange(
        "atomic-management.isActive",
        ({ newValue, oldValue }) => {
          this.enabledChanged(newValue);
        }
      ),
      ...
      atom.workspace.observeTextEditors(editor => {
        editor.onDidSave(e => {
          ...
          if (
            this.enabledProjects.length === 0 ||
            (this.enabledProjects.includes(e.path) &&
              e.path.endsWith(FILE_EXTENSION))
          ) {
            this.setConfig();
          }
        });
  ...
  setConfig() {
    ...
    if (
      this.defaultConfig &&
      this.defaultConfig.every(file => {
        return this.projectConfigs.includes(file);
      })
    ) {
  ...
  readConfigs(
    ...
    CSON.writeFile(savedConfigFile, atom.config.get(), () =>
      console.log("error")
    );
  },
