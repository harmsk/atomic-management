...
export default {
  ...
  readConfigFile(configName) {
    try {
      if (configName == 'atomconfig.cson') {
          contents = CSON.readFileSync(configName)
      } else if (configName == 'atomconfig.json') {
      ...
      default:
        ...
        if(this.defaultConfig && projectPaths.some(defaultCheck(this.defaultConfig))) {
          ...
          atom.confirm({
            ...
            '\tYour choice is remembered until you quit Atom. You can always reset the choice in "Packages -> atomic-management -> Reset remembered config"',
            buttons: buttons,
