...
export default {
  ...
  if(!map) {
    map = this.projectToConfigContentsMap
  }
   ...
   *                            or ".json"
   */
  readConfigFile(configName, set=true) {
    let contents;
    ...
    if (contents && !this.projectToConfigContentsMap.has(configName)) {
      // let configuredFields= new Map();
      ...
      //       configuredFields = configuredFields.set(
      //         keyPath,
      ...
      //   });
      // });
      // console.log(configName, contents)
      // this.projectToConfigContentsMap = this.projectToConfigContentsMap.set(configName, configuredFields);
      let configuredFields = this.addConfigToMap(configName,contents)
      if(set) {
        this.configuredFields=configuredFields;
        this.setSettings(configName, contents)
      }
    }
},

setSettings(configName, contents) {
    if (contents === null) {
      contents = {};
    }
        ...
        response => {
          ...
          if (response === buttons.length - 1) {
            ...
            this.projectConfigs.forEach(configName => {
              var set = configName==this.projectConfigs[response];
              this.readConfigFile(configName, set);
            });
  ...
  addConfigToMap(configName, contents) {
      var configuredFields = new Map();
        if (contents) {
          Object.keys(contents).forEach(namespace => {
            Object.keys(contents[namespace]).forEach(key => {
  ...
  mergeConfigs(newProjectConfigsArray) {
    ...
    this.projectConfigs.forEach(configName => {
      ...
      try {
        ...
        contents = this.standardizeConfig(contents);
        if(contents && !this.projectToConfigContentsMap.has(configName)) {
          this.addConfigToMap(configName, contents)
        }
