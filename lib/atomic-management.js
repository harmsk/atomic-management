...
export default {
    ...
    this.subscriptions.add(
      ...
      atom.workspace.observeTextEditors(editor => {
        editor.onDidSave(e => {
          ...
          if (this.projectConfigs.length === 0 || this.projectConfigs.includes(e.path)) {
            this.setConfig();
          }
        });
      })
  ...
  /**
   * Opens the primary .atom/config.(cson|json) file that is being applied to
   * the current window.
   */
  openConfigFile() {
   ...
   */
  openAllConfigFiles() {
    for (let i = this.projectConfigs.length - 1; i >= 0; i--) {
      atom.workspace.open(this.projectConfigs[i]);
    }
  },
