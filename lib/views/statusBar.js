          ...
          return (<div
              ...
              <div id={index} className='am-label'>
                <div>
                  ...
                  <a onClick={() => {atom.workspace.open(keyO)}}>{key} Local Settings - {name}</a>
                  <i className={`icon-arrow-down ${lightenDown}`} onClick={this.onDownArrowClick(index).bind(this)}/>
                  <i className={`icon-arrow-up ${lightenUp}`} onClick={this.onUpArrowClick(index).bind(this)}/>
                </div>
              </div>
          </div>)
      })
      ...
      ref={(e) => {this.status_ref=e}}>
          ...
          <div
              ...
              <div className='am-label'>
                <div>
                  <input type='checkbox' id='myCheckGlobal' checked={this.state.checked[this.state.checked.length - 1]} onChange={() => {return false}}/>
                  <a onClick={() => {atom.workspace.open(globalConfig)}}> Global Settings - {globalName}</a>
                </div>
              </div>
          </div>
      </div>
