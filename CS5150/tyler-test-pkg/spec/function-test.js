...
describe('TylerTestPkg', () => {
  ...
  describe('when change the folder is changed', () => {
    it('should change the global config file', () => {
      const globalConfigFilePath = '~/.atom/config.cson';
      ...
      if (fs.existsSync(globalConfigFilePath)) {
          const globalConfig = JSON.parse(fs.readFileSync(globalConfigFilePath, "utf8"));
      }
      Object.entries(config.settings).forEach(([key, val]) =>
        expect(globalConfig.key.toEqual(val)));
    });
  });
